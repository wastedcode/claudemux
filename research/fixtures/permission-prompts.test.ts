...
// the research harness (sandbox HOME + private tmux socket + sentinel mtime
// guard). Asserts the recorded `promptTextSnippet` still appears in the
// captured pane when the corresponding trigger fires.
//
// Per-scenario timeout: 30s. Output cap: 64 KB.
//
// Reference: research/findings.md §4.
    ...
    it(
      ...
      async () => {
        ...
        try {
          ...
          tmux(
            'new-session', '-d', '-s', scenario.id, '-x', '120', '-y', '40',
            'env', '-i',
            `HOME=${sandboxHome}`,
