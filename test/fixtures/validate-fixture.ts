  ...
  fixture.scenarios.forEach((s, i) => {
    ...
    if (tool === "Write" || tool === "Edit" || tool === "Read") {
      ...
      if (typeof cmd !== "string") {
        ...
        if (!BASH_ALLOWED_VERBS.has(first)) {
          fail(
            `${where}: trigger.args.cmd first token "${first}" not in allow-list ${[...BASH_ALLOWED_VERBS].join(", ")}`,
          );
        }
      }
    ...
    if (s.expectsPrompt) {
      if (typeof s.promptTextSnippet !== "string" || s.promptTextSnippet.length < 20) {
        fail(
          `${where}: promptTextSnippet must be a string of ≥ 20 chars when expectsPrompt is true`,
        );
      }
    }
